...
\begin{abstract}
    ...
    This provides a reduction to mean-payoff games, a well-studied class of problems in algorithmic game theory. It enables computing an optimal separating hyperplane via scalable iterative algorithms, with complexity polynomial in the size of the dataset and pseudo-polynomial in the desired precision.
    Finally, we combine tropical classifiers with linear feature maps to construct flexible piecewise-linear classifiers.
\end{abstract}
\todo[color=red!30]{SG: we forgot the implied constant in the KM algorithm, this leads to a complexity in $pd^2$ instead of $pd$, hence not linear in the data set.}
...
Nonetheless, large instances can be solved efficiently by iterative schemes such as relative Krasnoselskii--Mann iteration, which require $O(1/\varepsilon^2)$ operator evaluations, where the implied constant depends linearly
in the number of states of the game. \todo{SG: we said linear in the input size but this is not true}
...
\begin{enumerate}
    ...
    \item We develop an algorithm based on mean-payoff games and Krasnoselskii--Mann iteration that computes the optimal classifier in a time that is polynomial in the dimension and in the number of vectors of the data set.\todo{SG: we said linear but caveat, this was wrong, we get $pd^2$ as the detailed analysis below reveals}
...
Moreover, both $\rho(T)$ and a vector $a$ satisfying $T(a) = \rho(T)+a$ up to a precision $\epsilon$ can be computed up in a time that is polynomial in the dimension and in the number of vectors of the data set and pseudopolnomial in $\epsilon$ using mean-payoff game algorithms (see Section~\ref{sec:algorithm}).\todo{SG: linear $\to$ polynomial, rewritten, we oversold a bit!}
\end{theorem}
...
In contrast, Theorem~\ref{thm:spectral_separability} leads to a complexity quadratically bounded in $d$ (experiments reveal an even more favorable practical behavior, linear in $d$).\todo{SG: I was more precise, we said exponential but better to say in which variable. And linear was wrong in theory... but true in practice ? }
...
= \mathcal{O}(1/k^{1/2})$, where the constant implied in the $\mathcal{O}(\cdot)$ is explicit, of order $dW$ where $W=\max_{i,k} \|X^k_{\cdot,i}\|_H$,\todo{SG: warning a factor $n$ is hidden in the constant since the constant is in $\|u\|_H$ where $u$ is an eigenvector, the latter quantity is in $O(dW)$. This is not so obvious though (the proof I know is to use blackwell optimality)}
...
in the number of entries $pd$ of the training set\todo{SG: rewrote (we said the size but the size is in the turing model, ie $\log W$ is part of the size)}
