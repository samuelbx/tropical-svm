...
Nonetheless, large instances can be solved efficiently by iterative schemes such as relative Krasnoselskii--Mann iteration, which require $O(1/\varepsilon^2)$ operator evaluations, where for the subclass of games under consideration
in this paper, the implied constant depends on the maximal absolute
value of the entries of the data points but not on the number of data points or dimension.
\todo{SG: we said linear in the input size but imput size is ambiguous. Actually, in general, number of iteration of KM linear in the number of states, but the matrices are full, hence a small trick (new lemma in the appendix) implies the number of iteration is in $O(W)$. SO I rewrote}
...
\begin{enumerate}
    ...
    \item We develop an algorithm based on mean-payoff games and Krasnoselskii--Mann iteration that computes the optimal classifier in a time that is linear in the dimension and in the number of vectors of the data set.\todo{SG: more precised explanation about linearity}
...
originate from~\cite{AGGut10} in which they were used to check tropical linear independence,\todo{SG: added original ref and explanation}
they were further used in~\cite{akiangaubertqisaadi},
to solve tropical linear regression
problems.
...
\end{figure}
\todo[inline]{SG: SB, TM, I see an orange point in the black region
in Figure~\ref{fig:tropical_poly}, we should explain somewhere how it was handled. Is it an outlier handled by some heuristic?}
...
Actually, we shall need the following ``diagonal-free'' variant of the canonical projection~\eqref{e-canonical},  introduced in~\cite[Section~4.2]{AGGut10}
\todo{SG: added original ref; interestingly, the DF map allows for a quantitative check of tropical linear independence.}
(up to a change of sign)
and further studied in~\cite[Equation~(9)]{akiangaubertqisaadi}:
\begin{align}
...
In particular, when $T(u)=\lambda + u$
for some $u\in \R^d$ and $\lambda\in \R$, then, $\lambda=\rho(T)$
and the ball (in Hilbert seminorm)
centered at $u$ of radius $\rho(T)$ is included
in $\tcone(X)$. Moreover, $\rho(T)$ is the maximal radius
of a ball included in $\tcone(X)$.\todo{SG: added this sentence to provide more insight}
...
corresponding to the sectors assigned to each class, such that for every class $1 \leq k \leq K$, every point $x \in X^k$, belongs to the union $S^k$ of the sectors $(\text{Sec}_i(a))_{i\in I^k}$ assigned to class $k$. Then, it follows that the whole tropical cone $V^k=\tcone(X^k)$ is also included in $S^k$.

Moreover, we say that the hyperplane separates {\em with margin} $\gamma > 0$ if, for any class $k$ and any point $x \in V^k$, the Hilbert distance from $x$ to the union of complementary sectors $\bar{S}_k= \cup_{i\not\in I^k} \text{Sec}_i(a)$
...
(3) \textit{Soft-Margin Interpretation:} For non separable data, and for the binary classification problem, $\rho(T)$ is positive and coincides with the minimal distance of the original data $(X^1,X^2)$ to perturbed data $(\tilde{X}^1,\tilde{X}^2)$ such that $\tcone (\tilde{X}^1)\cap \tcone(\tilde{X}^2)$ is of non-empty interior. 
\todo{SG: replaced ``that nullifies $\rho(T)$.'' by a more informative geometric statement. For $\rho(T)=0$ is more difficult to interpret. An illustrative example should be given}

Moreover, both $\rho(T)$ and a vector $a$ satisfying $T(a) = \rho(T)+a$ up to a precision $\epsilon$ can be computed up in a time that is linear in the dimension and in the number of vectors of the data set and pseudopolnomial in $\epsilon$ using mean-payoff game algorithms (see Section~\ref{sec:algorithm}).\todo{SG: explained mrore precisely linearity}
\end{theorem}
...
Our framework advances the tropical SVM foundation established by Gärtner and Jaggi \cite{gartner2008}. Their work showed that tropical SVM could be formulated as finding a point of maximum margin within a tropical polytope defined by sector constraints, but required exhaustive exploration of sector assignments, leading to a complexity in $\Omega(2^d)$.
In contrast, Theorem~\ref{thm:spectral_separability} leads to a complexity linearly bounded in $d$ \todo{SG: I was more precise, we said exponential but better to say in which variable. }
In fact, our spectral characterization offers a complete theoretical understanding of when data are tropically separable, leading to an exact formula for the optimal margin, an efficient algorithm to find the optimal classifier without combinatorial exploration and a natural interpretation for non-separable cases.
...
\todo{SG: Add a comment explaining the optimality of the soft-margin interpretation}
...
= \mathcal{O}(1/k^{1/2})$, where the constant implied in the $\mathcal{O}(\cdot)$ is explicit, of order $W$ where $W=\max_{i,j,k} |X^k_{i,j}|$,\todo{SG: We forgot to make explicit the constant which could have led to a mistake, because the constant is generally in $dW$, but there is a hack exploiting the full character which allows to us to reduce to $W$, eventually justifying our claim to be linear in the dimension, see the new lemma in the appendix}
see~\cite{akianmfcs} for details, and the discussion of the appendix.
Hence, Algorithm~\ref{alg:km_iteration} terminates in $O(W/\varepsilon^2)$ iterations, independently of the dimension $d$ or of the number of data points.\todo{SG: emphasized}
...
$\mathcal{O}(pd^2W)$ time, where $p=p_1+\dots+p_K$ is the total number of points.
Hence, Algorithm~\ref{alg:tropical_svm} terminates in $O(pd^2W/\varepsilon^2)$ time. Observe
that this bound is {\em polynomial} in the number of entries $pd$.
...
For the first time, we demonstrate that tropical SVMs can be practically computed with polynomial guarantees, removing the exponential complexity that limited previous approaches.\todo{SG: rewrote exponential barrier}
While specialized classical SVM libraries remain faster on conventional tasks, our approach enables new applications where piecewise-linear decision boundaries offer advantages.
...
We addressed a key computational barrier limiting tropical SVM's practical application in machine learning. By reformulating tropical classification through spectral theory and mean-payoff games, we reduced complexity from exponential to quadratic in the dimension (but pseudo-polynomial in the desired precision),
...
\begin{proof}
Suppose that the hyperplane $\mathcal{H}_a$ separates $X^1,\ldots,X^K$ with a margin of at least $\gamma > 0$. Consider a class $k$, and let $I^k$ denote the set of coordinates assigned to class $k$. For any point $x \in X^k$, the margin condition implies that its Hilbert distance to its non-assigned sectors $\bar{S}^k$ is not less than $\gamma$, namely
\begin{align}
d_H(x, \bar{S}^k) = \max_j(x_j - a_j) - \max_{i \not\in I^k}(x_i - a_i) \geq \gamma.
\end{align}
...
This motivates the following abstract result, which applies in particular to $V^k=\tcone(X^k)$.


\begin{lemma}[Max-margin hyperplane]\label{lemma:operator_to_hyperplane}
  Let $T^1,\dots,T^K$ be Shapley operators such that all the images $\{T^k(x)\mid x\in \R^d\}$ with $1\leq k\leq k$ are bounded
  in Hilbert seminorm, let $V^k=\mathcal{S}(T^k)$,
and let $T$ be defined as per~\eqref{eq:single_operator}.
  If $\rho(T) < 0$, then there exists a hyperplane $\mathcal{H}_a$ that separates the tropical cones $V^1,\dots,V^k$
...
$(T^k(x) - T^k(a))_i \leq \max_j(x_j - a_j)$.
...
With $\rho(T) < 0$, this implies $x_i - a_i < \max_j(x_j - a_j)$, implying that $i$ cannot belong to the $\arg\max$ of $x - a$. Therefore, the $\arg\max$ must lie in $I^k$, placing $x$ in one of its assigned sectors. In particular, $I^k$ is non-empty.

Finally, for the margin, consider $x \in \mathcal{S}(T^k)$: 
\begin{align}
d_H(x, \bar{S}^k) = \max_j(x_j - a_j) - \max_{i \not\in I^k}(x_i - a_i).
\end{align}

Using the inequalities derived above, we get $d_H(x, \bar{S}^k) \geq -\rho(T)$.
\end{proof}
...
\todo{SG: issue to be fixed quietly after submission to NEURIPS, Theorem 16 (originally proved by XA, SG, Katz) was reproduced in \cite{allamigeon_condition} without proof, it is an issue to quote a result whose proof is not available}
...
\begin{lemma}[Overlap interpretation in the binary classification problem]\label{lemma:perturbation}
  Let $T^k$ be the diagonal-free projection of $\tcone(X^k)$, for $k=1,2$.
  Let $T=\min(T^1,T^2)$, and suppose that $\rho(T) \geq 0$. Then, there exists a perturbation of the point sets $X^1$ and $X^2$, with each point moved by at most $\rho(T)$ in the tropical metric, such that the tropical convex hulls of the perturbed sets have a trivial intersection.
\end{lemma}
\begin{proof}
...
\end{proof}
The following result allows us to bound $\rho(T)$ as well as an eigenvector $u$ such that $T(u) = \rho(T) +u$.\todo{SG: new lemma added. This is needed to achieve the linear bound}
\begin{lemma}\label{lemma-short}
  Let $W=\max_{k,i,j}|X^k_{ij}|$. Then, $|\rho(T)|\leq $, and
  any vector $u\in \R^d$ such $T(u) = \rho(T)+u$ satisfied $\|u\|_H\leq 2W$.
\end{lemma}
\begin{proof}
  We observe that every vector of $X^1\cup\dots \cup X^K$
  is included in the ball $B(0,2W)$ in Hilbert's seminorm.
  Since this ball is stable by tropical linear combinations,
  it follows that the sets $\tcone(X^1),\dots,\tcone(X^k)$ are all
  included in $B(0,W)$. Now, we see from the formula~\eqref{e-def-DF}
  defining $P_X^{\text{DF}}$ that the range of $P_X^{\text{DF}}$ is included
  in $\tcone(X)$. So, all the ranges of $P_{X^1}^{\text{DF}},\dots, P_{X^K}^{\text{DF}}$
  are included in $B(0,2W)$. Since $B(0,2W)$ is not only stable by sup,
  but also by inf, it follows that for all $y\in\R^d$, $T(y) =\max_{k<l}\min(P_{X^k}^{\text{DF}}(y), P_{X^l}^{\text{DF}}(y))\in B(0,2W)$. This implies that
  any eigenvector $u\in \R^d$ of $T$ belongs to $B(0,2W)$. Since the eigenspace
  of $T$ is preserved by adding constant vectors, we can choose an eigenvector such that $0\leq u_i \leq 2W$ holds for all $i\in [d]$. Then, Using the expression of $T$, together with \eqref{def-DF}, we deduce from $\lambda +u = T(u)$ that
  there exists $k,l,j$ and $\lambda + u_i = X^k_{ij}-X^k_{lj + u_j$, which entails that $|\lambda| \leq 6W$. Hence, $|\rho(T)|\leq 6W$.
  \end{proof}
...
Finally, the Baillon-Bruck theorem~\cite{baillonbruck} implies
that if $u$ is any eigenvector of $T$, then, Algorithm~\ref{}
terminates in $O(\|u\|_H/\epsilon^2)$ iterations. By Lemma~\ref{lemma-short},
this number of iterations is bounded in $O(W/\epsilon^2)$.
...
shows that the current interpreted (Python/NumPy) implementation is slower, by a factor $10^3$, than a state-of-the-art implementation of classical SVMs. However, Figures~\ref{fig:scaling_analysis} show that the execution time of our method scales linearly
in the number of entries $pd$ of the training set\todo{SG: replaced size by number of entries to be less ambiguous}
and in the number of features (monomials), thereby providing
