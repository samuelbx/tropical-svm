...
introduced in~\cite[Equation~(9)]{akiangaubertqisaadi} (up to a change of sign):\todo{SG: explained the change of sign}
\begin{align}
...
The maximum is always achieved. \todo{Théo : idem indique-t-on une source? ou est-ce clair que le résultat est dans une des trois sources citées ci-après? SG: c'est dans chacune des trois sources avec des degres de generalite et de precision different}
...
Consider a classification problem with $K$ classes, each represented by a set of points $X^1,\dots,X^K \subset \R^d$.\todo{SG: points should be {\em finite} (not in $\trop^d$, otherwise some statements may be more delicate -- or we need to check }
We define an operator $T^k$ for each class $k$ by taking the diagonal-free variant of the tropical projection onto the convex hull of points in that class:\todo[inline,color=red!30]{ SG: $T^k(x)$ is confusing owing to conflict with $x$ in $X^k$}
\[
...
\end{align}
Finally, with define the {\em distance} between a collection of perturbed point clouds $\tilde{X}^1,\dots, \tilde{X}^K$ and an original collection of point clouds $X^1,\dots,X^K$ as $\max_{1\leq k\leq k} \delta_H(X_k,\tilde{X}_k)$ where $\delta_H$ denotes the Hausdorff distance induced by Hilbert seminorm.\todo[color=red!30]{SG: defined distance of perturbed}
...
(3) \textit{Soft-Margin Interpretation:} For non separable data, and for the binary classification problem, $\rho(T)$ is positive and coincides with the minimal distance of the original data to perturbed data that achieve separability.\todo{SG: added more explanation/non-separated instances}
...
in~\cite{akianmfcs}.
\todo[color=red!30]{SG: again $T(x)$ is disturbing given $X$ is data cloud, can we write $T(y)$?}
\begin{algorithm}
...
\end{algorithmic}
\end{algorithm}
It follows from from a theorem of Baillon and Bruck~\cite{baillonbruck} that
as soon the eigenproblem $T(a)=\lambda + a$ with $a\in \R^n$ and $\lambda \in \R$ is solvable, which as observed above is always the case for the operator $T$ in~\eqref{eq:single_operator},\todo{SG: I added a justification}
we have
$\left\|T(x^{(k)})-x^{(k)}\right\|_H
...
\end{algorithmic}
\end{algorithm}
\todo[inline,color=red]{SG: sign issue in algo 2 at line 9 in $T^k(-a)_i > \rho(T) - a_i$, from the proof it seems it should be $T^k(a)_i > \rho(T) + a_i$. I corrected, please check!}
...
that this bound is {\em linear} in the input size $pd$. \todo{Théo : devrait-on comme dans la conclusion indiquer qu'on peut échanger l'algorithme 1 par un autre plus performant comme la policy-iteration? SG: I added it in the concluding section}
...
games in practice~\cite{chaloupka}.\todo{SG: ref to chaloupka}
...
\todo{SG: sub-fixed point not fixed-point}
...
Let $T^k$ be tropical projections on finite point clouds or their diagonal-free coutnerparts, so that $T^k = P_{X^k}$ or $T^k=\Pdiagfree_{X^k}$, and $T$ defined as the $\operatorname{\max}_2$ of the $T^k$, as in Equation \ref{eq:single_operator}.
...
$T^k=P_{X^k}\leq P\diagfree_{X^k}$,
\[
-\gamma  + a_i \geq [\Pdiagfree_{X^h}(a)]_i \geq [T^k(a)]_i\enspace .
\]
\todo[inline,color=red!30]{SG:  I didnot get the previous proof (double max at right is puzzling), so I detailed above -- previous proof
...
Finally, if $T(a)=\rho(T)+a$, then, by the Collatz-Wieland theorem (see~\cite[Theorem~7]{akiangaubertqisaadi}), $\rho(T)\le -\gamma$.\todo{SG: reference added}
\end{proof}
