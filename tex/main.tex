...
A tropical hyperplane with parameters $a=(a_1,\dots,a_n) \in \trop^d$ is defined as:
...
\end{align}
When the vector $a$ is finite, it is called the {\em apex}
...
\STATE \textbf{Assign Sectors to Classes:}
  ...
  \ENDIF
\STATE \textbf{Output:} Vector $a$ parametrizing the hyperplane, margin $-\rho(T)$ (if separable), and sector assignments for classification
\end{algorithmic}
\end{algorithm}
