...
\FOR{$k = 0, 1, 2, \ldots$ until convergence}
  ...
  \ENDIF
\ENDFOR
\STATE \textbf{Return:} $\rho(T) \approx \lambda^{(\text{final})}$, $a \approx x^{(\text{final})} - \frac{1}{d}\sum_{i=1}^d x_i^{(\text{final})} \cdot \mathbf{1}_d$ \COMMENT{Center the apex for numerical stability}
\end{algorithmic}
\end{algorithm}
