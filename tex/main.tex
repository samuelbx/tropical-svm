...
(1) \textit{Separability Criterion:} The data are separable by a tropical hyperplane if and only if $\rho(T) < 0$.\todo{SG: it should be explained what separable by a hyperplane exactly means (in terms of sectors)}

(2) \textit{Margin Optimality:} For separable data, the maximum margin equals $-\rho(T)$ and is achieved.\todo{SG explain what the margin means }

(3) \textit{Soft-Margin Interpretation:} For non separable data, and for the binary classification problem, $\rho(T)$ is positive and coincides with the minimal perturbation of the data, measured in Hilbert's seminorm, needed to achieve separability.\todo{SG: added more explanation/non-separated instances}
...
(relative value iteration), in a non-optimized way. We could rely instead on the policy iteration algorithm~\cite{DG-06} or its refinement allowing parallelization in~\cite{chaloupkathesis}, which have been reported to provided experimentally to fastest algorithm to solve practially mean-payoff
games~\cite{chaloupka}.\todo{SG: ref to chaloupka}
