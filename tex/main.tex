...
The theoretical fundation of this work relie on a separation theorem of Cohen, Gaubert and Quadrat~\cite{cohen2004}, refined in~\cite{AGNS10},  showing that the projection on tropical cone provides a nearest point in Hilbert's seminorm. This led to the development of the tropical
...
Christopoulos and Paragos investigated the separation by tropical polynomials.
A survey of the applications of tropical geometry to machine learning
can be found in~\cite{maragos2021}.
Paragos et al. studied several other problems of tropical linear regression.
A survey of the application of tropical geometry to regression problems can be found
in~\cite{}. Yoshida, and her coworkers, have developed a series of work
dealing with tropical approximation (principal component analysis, regression)
and separation (tropical SVM), and applied them to problems of phyologenetic analysis.
...
Tang et al.~\cite{tang2020} and Monod et al~\cite{monod2022} later developed specialized algorithms for binary classification cases where data points from the same category remain in the same sector, showing promising results in computational biology for analyzing evolutionary trees.\todo{quote lavishly yoshida}

\todo[inline]{SG: I will add references to AGG, Sergei, YangQi, Joswig (transport optimal tropical), Yue Ren/Montufar. Discuss Yoshida/heuristics more precisely; Paragos }
...
\]
where $[\cdot]$ denotes the equivalence class of a vector in  $\trop^d \setminus \{(-\infty,\dots,-\infty)\}$.\todo{SG: changed numbering}
...
of the data set, finding the optimal set of linear features
...
%% intelligent monomial selection strategies that optimize the expressivity-efficiency tradeoff.
...
%although its complexity is generally exponential.
%% Also, developing specialized parallel implementations would be needed to further accelerate computations, particularly for high-dimensional feature spaces or large-scale datasets with millions of points.

%, which are still to be investigated. 
\todo[color=red!30]{Remercier les eleves de l'an dernier}
...
\todo{This is the only place where diagonal free is needed.}
...
%% Figure~\ref{fig:scaling_analysis} reveals that the tropical SVM algorithm exhibits linear scaling behavior both with respect to the number of effective monomials and the training set size.
%% We used a Python script that systematically varies PCA dimensions, polynomial degrees, and sample sizes on MNIST data \cite{MNIST}.
