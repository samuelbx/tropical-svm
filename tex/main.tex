...
It has been successfully applied to linear regression \cite{maragos2020,akian2020} \todo{Théo : la seconde référence n'est pas plutôt \cite{akiangaubertqisaadi} (trop lin reg)?}, principal component analysis \cite{yoshida2019} and neural network analysis \cite{maragos2021}.
...
\todo[inline]{SG: I will add references to AGG, Sergei, YangQi, Joswig (transport optimal tropical), Yue Ren/Montufar. Discuss Yoshida/heuristics more precisely; Maragos }
...
\todo{Théo : peut-être ajoute-t-on une remarque disant qu'on n'utilisera plutôt dans la suite du papier les notations usuelles + et max ? Puisqu'on utilise qu'ici les opérations tropicales + même question pour l'espace projectif PT qu'on ne mentionne que ci dessous?}
...
A tropical hyperplane with parameters $a=(a_1,\dots,a_n) \in \trop^d$\todo{the apex is usually $-a$ and this is defined only for $a\in \R^n$, $a$ is rather a vector of parameters} is defined as:
\begin{align}
...
\end{align}
When the vector $a$ is finite, the vector $-a$ is called the {\em apex}
of the tropical hyperplane, for each of these sections is a cone
originating from point $-a$.\todo[inline,color=red!30]{SG: New/consistent terminology}
...
precisely $[P_X(y)]_i$. \todo{Théo : devrait-on indiquer une source pour ce résultat ? idem pour le fait que P et Pdf ont les mêmes points fixes (même si la preuve se fait en deux lignes)?} We have 
...
The ``diagonal-free'' constraints imply that the opponent is prevented from replying tit-for-tat.
...
\STATE \textbf{Compute Eigenpair:}
  \STATE \quad Run KM iterations (Algorithm \ref{alg:km_iteration}) to obtain $\rho(T)$ and a vector $a$ parametrizing an hyperplane with accuracy $\epsilon>0$.
\STATE \textbf{Assign Sectors to Classes:}
  ...
  \ENDIF
\STATE \textbf{Output:} Vector $a$ (so that $-a$ is the apex of the separating hyperplane), margin $-\rho(T)$ (if separable), and sector assignments for classification
\end{algorithmic}
\end{algorithm}
...
The following Lemma is the only one where using the diagonal-free operator is needed.\todo{Théo : d'après ma remarque ci-dessus, on devrait alors indiquer plus globalement et plus simplement que la version df est uniquement utile pour la compréhension du cas binaire non séparable}
