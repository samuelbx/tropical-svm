...
\todo[inline]{SG: I will add references to AGG, Sergei, YangQi, Joswig (transport optimal tropical), Yue Ren/Montufar. Discuss Yoshida/heuristics more precisely}
...
The tropical projection $P_X(y)$ of a point $y$ onto this tropical cone is
defined by:
\begin{align}
...
%% \[
%% [P_X(y)]_i = \max_{1 \leq j \leq p} \left\{X_{ij} + \min_{1 \leq k \leq d} \left(-X_{kj} + y_k\right)\right\}\enspace,
%% \]
...
\todo[inline]{Compare with \cite{gaubert2011}}
%% One can verify
%% the triviality of the intersection of two tropical cones by projecting alternatively on
...
%% The first step is to compute the projections efficiently.
%% The projection $P_X(y)$ of a point $y$ onto the tropical cone generated by a
%% subset $X \subset \trop^d$ 
...
$\operatorname{tcone}(X)= \{y\in \trop^d\mid y\leq P_X^{\text{DF}}(y)\}$, and similarly
for $P_X$, so both operators can be used to ``represent'' $\operatorname{tcone}(X)$.
...
To compute the spectral radius $\rho(T)$ and a corresponding eigenvector $a$, we will
rely on Algorithm~\ref{alg:km_iteration}, a modification of relative value
iteration using Krasnoselskii-Mann damping, studied
in~\cite{akianmfcs}.

\begin{algorithm}
\caption{Krasnoselskii--Mann Iteration with Krasnoselskii-Mann Damping~\cite{akianmfcs}}\label{alg:km_iteration}
\begin{algorithmic}[1]
...
\REPEAT%{$k = 0, 1, 2, \ldots$ until convergence}
...
%  \lambda^{(\text{final})}$,
  $a \approx x^{(k+1)}$. % - \frac{1}{d}\sum_{i=1}^d x_i^{(\text{final})} \cdot \mathbf{1}_d$
\end{algorithmic}
\end{algorithm}
...
%Recent results by Allamigeon et al. \cite{allamigeon2025} show that
Hence, Algorithm~\ref{alg:km_iteration} terminates in $O(1/\varepsilon^2)$ iterations.
%% By avoiding the combinatorial exploration of sector assignments, our method runs in pseudo-polynomial time—specifically $O\bigl(\frac{nd}{\varepsilon^2}\bigr)$ for $n$ data points in $d$ dimensions.
...
%$X—linear in both the number of points and dimensions.
