...
\end{algorithmic}
\end{algorithm}
\todo[inline,color=red]{SG: sign issue in algo in $T^k(-a)_i > \rho(T) - a_i$, from the proof it seems it should be $T^k(a)_i > \rho(T) + a_i$}
...
%% or equivalently
%% \begin{align}
%%   a_i \leq \min_{x\in X^k} 
%% %  \max_{x \in X^k}(x_i - a_i) \leq
%% \Big(  -x_i +\max_{x \in X^k}\big(\max_{j\neq i}(x_j - a_j)\big)\Big) - \gamma.
%% \end{align}
%% %% 
