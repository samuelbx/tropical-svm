...
Since this holds for all classes $k \neq \ell$, we have $-\gamma + a_i \geq T(a)_i$, for all $i\in I^\ell$. Since $\{1,\dots,d\}=I^1\cup\dots\cup I^K$, we deduce that $-\gamma +a\geq T(a)$.\todo{SG: Use explicitely the covering condition}
Then, we deduce from
the Collatz-Wielandt theorem (see~\cite[Theorem~7]{akiangaubertqisaadi})
that 
%Finally, if $(\rho(T),a)$ is an eigenpair, $T(a)_i=\rho(T)+a_i$, hence
$-\gamma\geq \rho(T)$.\todo{SG: added ref to CW theorem}
\end{proof}

\begin{lemma}[Max-margin hyperplane]\label{lemma:operator_to_hyperplane}
  Let $T^1,\dots,T^K$ be arbitrary Shapley operators, and $T$ defined as per~\eqref{eq:single_operator}.\todo{SG: make explicit that the $T^k$ are arbitrary}
  If $\rho(T) < 0$, then there exists a hyperplane $\mathcal{H}_a$ that separates the sub-fixed-point sets $\mathcal{S}(T^k)$ with a margin of at least $-\rho(T)$.\todo{SG: we defined separation only for $\tcone(X^k)$, not for any sub-fixed-point set, we will have to fix this}
\end{lemma}
...
\begin{lemma}[Overlap interpretation in the binary classification problem]\label{lemma:perturbation}
  ...
  Let $T=\min(T^1,T^2)$, and suppose that $\rho(T) \geq 0$. Then, there exists a perturbation of the point sets $X^1$ and $X^2$, with each point moved by at most $\rho(T)$ in the tropical metric, such that the tropical convex hulls of the perturbed sets have a trivial intersection.\todo{replaced empty by trivial for $-\infty$ is always in the intersection}
\end{lemma}
...
\begin{equation}
[x^{c}]_{i}\leq T^{c}(x^{c})_{i}=\underbrace{\left(T^{c}(x^{c})-T^{c}(a)\right)_{i}}_{\leq\,\max_{k\ne i}(x_{k}^{c}-a_{k})}+T^{c}(a)_{i}.\label{eq:major}
\end{equation}
...
hence $\tilde{x}_{jk}-\max_{k\ne i}(\tilde{x}_{jk}-a_{k})\leq a_{i}$.
Otherwise, $x_{j}$ is in the $i$-th sector and $\max_{k\ne i}(\tilde{x}_{jk}-a_{k})=x_{jt}-a_{t}$,
thus
\[
\tilde{x}_{ji}-\max_{k\ne i}(\tilde{x}_{j}-a)_{k}=(\tilde{x}_{j}-a)_{i}-(\tilde{x}_{j}-a)_{t}+a_{i}\geq a_{i},
\]
with equality if $d_{H}\left(x_{j},\mathcal{H}_{a}\right)\leq\rho(T)$.

Suppose by symmetry that $T(a)_{i}=T^{1}(a)_{i}=\rho(T)+a_{i}$. We
also have $T^{2}(a)\geq\rho(T)+a_{i}$. Then, using the proof of Theorem
...
and $\tilde{x}_{j_{2}i}-\max_{k\ne i}(\tilde{x}_{j_{2}k}-a_{k})\geq a_{i}$.
Moreover, for any $j$ such that $x_{j}\in X^1$ is in sector $i$,
Equation \ref{eq:major} gives $d_{H}(x_{j},\mathcal{H}_{a})\leq\rho(T)$.
